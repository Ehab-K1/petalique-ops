...
/*
 * Public order form. Products and add-ons come from the database catalog
 * (Settings → Product catalog), so the studio can change its menu without
 * touching code.
 */
export default function PublicOrderForm({ products, addons, business }) {
              ...
              <div className="chip-group">
                {products.map((p) => (
                  <label key={p.name} className={"chip" + (form.products.includes(p.name) ? " on" : "")}>
                    <input type="checkbox" checked={form.products.includes(p.name)}
                      onChange={() => toggle("products", p.name)} />
                  ...
                  <div className="chip-group">
                    {addons.map((a) => (
                      <label key={a.name} className={"chip" + (form.addons.includes(a.name) ? " on" : "")}>
                        <input type="checkbox" checked={form.addons.includes(a.name)}
                          onChange={() => toggle("addons", a.name)} />
                        {a.name}{a.price_label ? ` (${a.price_label})` : ""}
                      </label>
                    ))}
                  </div>
                  <label className="field">
